...
import 'package:talktsy/utils/voip_plugin.dart';
import 'package:universal_html/html.dart' as html;
import 'package:get/get.dart';
import 'package:talktsy/utils/client_manager.dart';
import 'package:talktsy/utils/init_with_restore.dart';
import 'package:talktsy/utils/localized_exception_extension.dart';
import 'package:talktsy/utils/matrix_sdk_extensions/matrix_file_extension.dart';
import 'package:talktsy/utils/platform_infos.dart';
import 'package:talktsy/utils/uia_request_manager.dart';
...
class MatrixState extends State<Matrix> with WidgetsBindingObserver {
  ...
  Client getLoginClient() {
    ...
    final candidate = _loginClientCandidate ??= ClientManager.createClient(
      '${L10n.of(context)?.appName ?? ""}-${DateTime.now().millisecondsSinceEpoch}',
    )..onLoginStateChanged
          .stream
          .where((l) => l == LoginState.loggedIn)
          .first
          .then((value) {
        log('LoginState $value');
  ...
  Future<void> dehydrateAction() async {
    ...
    final exportFileName =
        'talktsy-export-${DateFormat(DateFormat.YEAR_MONTH_DAY).format(DateTime.now())}.fluffybackup';
