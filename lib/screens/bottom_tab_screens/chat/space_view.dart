...
import 'package:talktsy/screens/bottom_tab_screens/chat/search_title.dart';
import 'package:talktsy/utils/localized_exception_extension.dart';
import 'package:talktsy/utils/stream_extension.dart';
import 'package:talktsy/widgets/avatar.dart';
import 'package:talktsy/widgets/matrix.dart';
import 'package:talktsy/widgets/public_room_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:talktsy/utils/adaptive_bottom_sheet.dart';
...
class _SpaceViewState extends State<SpaceView> {
  ...
  void _addChatOrSubspace() async {
    final roomType = await showConfirmationDialog(
      context: context,
      title: L10n.of(Get.context!)?.addChatOrSubSpace ?? "",
      actions: [
        AlertDialogAction(
          key: AddRoomType.subspace,
          label: L10n.of(Get.context!)?.createNewSpace ?? "",
        ),
        AlertDialogAction(
          key: AddRoomType.chat,
          label: L10n.of(Get.context!)?.createGroup ?? "",
        ),
      ],
    );
    ...
    final names = await showTextInputDialog(
      ...
      title: roomType == AddRoomType.subspace
          ? L10n.of(Get.context!)?.createNewSpace ?? ""
          : L10n.of(Get.context!)?.createGroup ?? "",
      textFields: [
        DialogTextField(
          hintText: roomType == AddRoomType.subspace
              ? L10n.of(Get.context!)?.spaceName ?? ""
              : L10n.of(Get.context!)?.groupName ?? "",
          minLines: 1,
          maxLines: 1,
          maxLength: 64,
          validator: (text) {
            if (text == null || text.isEmpty) {
              return L10n.of(Get.context!)?.pleaseChoose ?? "";
            }
            return null;
          },
        ),
        DialogTextField(
          hintText: L10n.of(Get.context!)?.chatDescription ?? "",
          minLines: 4,
          maxLines: 8,
          maxLength: 255,
        ),
      ],
      okLabel: L10n.of(Get.context!)?.create ?? "",
      cancelLabel: L10n.of(Get.context!)?.cancel ?? "",
    );
  ...
  Widget build(BuildContext context) {
    ...
    final displayname = room?.getLocalizedDisplayname() ??
        L10n.of(Get.context!)?.nothingFound ??
        "";
    return Scaffold(
      appBar: AppBar(
        leading: Center(
        ...
        title: ListTile(
          ...
          subtitle: room == null
              ? null
              : Text(
                  'countChatsAndCountParticipants'.trParams({
                    'chats': '${room.spaceChildren.length}',
                    'participants': '${room.summary.mJoinedMemberCount ?? 1}',
                  }),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
              ...
              PopupMenuItem(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Text(L10n.of(Get.context!)?.invite ?? ""),
                  ],
                ),
              ),
              PopupMenuItem(
                  ...
                  slivers: [
                    SliverAppBar(
                      ...
                      title: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          contentPadding: EdgeInsets.zero,
                          hintText: L10n.of(Get.context!)?.search ?? "",
                          hintStyle: TextStyle(
                        ...
                        if (i == 0) {
                          return Column(
                            ...
                            children: [
                              ...
                              SearchTitle(
                                title: L10n.of(Get.context!)?.joinedChats ?? "",
                    ...
                    SliverList.builder(
                      ...
                      itemBuilder: (context, i) {
                        if (i == 0) {
                          return SearchTitle(
                            title: L10n.of(Get.context!)?.discover ?? "",
                        ...
                        if (i == _discoveredChildren.length) {
                          if (_noMoreRooms) {
                            return Padding(
                              ...
                              child: Center(
                                child: Text(
                                  L10n.of(Get.context!)?.noMoreChatsFound ?? "",
                          ...
                          return Padding(
                            ...
                            child: TextButton(
                              ...
                              child: _isLoading
                                  ...
                                  : Text(L10n.of(Get.context!)?.loadMore ?? ""),
                            ),
                          );
                        }
                        ...
                        return Padding(
                          ...
                          child: Material(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text(
                                item.topic ??
                                    'countParticipants'.trParams({
                                      'count': '${item.numJoinedMembers}',
                                    }),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
