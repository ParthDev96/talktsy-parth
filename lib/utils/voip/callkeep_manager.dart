...
class CallKeepManager {
  ...
  Future<CallKeeper> displayIncomingCall(CallSession call) async {
    ...
    await _callKeep.displayIncomingCall(
      call.callId,
      '${call.room.getLocalizedDisplayname()} (Talktsy)',
      localizedCallerName: '${call.room.getLocalizedDisplayname()} (Talktsy)',
      handleType: 'number',
  ...
  Future<void> checkoutPhoneAccountSetting(BuildContext context) async {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text(L10n.of(Get.context!)?.callingPermissions ?? ""),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ListTile(
              onTap: () => openCallingAccountsPage(context),
              title: Text(L10n.of(Get.context!)?.callingAccount ?? ""),
              subtitle:
                  Text(L10n.of(Get.context!)?.callingAccountDetails ?? ""),
            ...
            ListTile(
              ...
              title: Text(L10n.of(Get.context!)?.appearOnTop ?? ""),
              subtitle: Text(L10n.of(Get.context!)?.appearOnTopDetails ?? ""),
              trailing: const Icon(Icons.file_upload_rounded),
            ),
            const Divider(),
            ListTile(
              onTap: () => openAppSettings(),
              title: Text(L10n.of(Get.context!)?.otherCallingPermissions ?? ""),
