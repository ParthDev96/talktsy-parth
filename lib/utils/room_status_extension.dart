...
extension RoomStatusExtension on Room {
  String getLocalizedTypingText(BuildContext context) {
    ...
    if (typingUsers.length == 1) {
      typingText = L10n.of(Get.context!)?.isTyping ?? "";
      if (typingUsers.first.id != directChatMatrixID) {
        typingText = 'userIsTyping'
            .trParams({'username': typingUsers.first.calcDisplayname()});
      }
    } else if (typingUsers.length == 2) {
      typingText = 'userAndUserAreTyping'.trParams({
        'username': typingUsers.first.calcDisplayname(),
        'username2': typingUsers[1].calcDisplayname(),
      });
    } else if (typingUsers.length > 2) {
      typingText = 'userAndOthersAreTyping'.trParams({
        'username': typingUsers.first.calcDisplayname(),
